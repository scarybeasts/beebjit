...
enum {
  ...
  k_video_timer_expect_vsync_raise = 1,
...
video_read_data_byte(struct video_struct* p_video,
  ...
  if (address_counter & 0x2000) {
     ...
     * check is inverted.
     * In both teletext and 1MHz bitmap mode, latching is performed such that
     * the non-xor'ed memory location is used.
     */
    if (is_teletext) {
      /* No address xor. */
    } else if (!p_video->is_ula_clock_fast) {
      /* No address xor. */
    } else {
      /* 2MHz, bitmap mode. */
      if (!(ticks & 1)) {
...
video_start_new_frame(struct video_struct* p_video) {
  ...
  p_video->had_odd_vsync_this_row = 0;
...
static void
video_update_VSYNC(struct video_struct* p_video, uint64_t ticks) {
  struct via_struct* p_system_via;
  int is_new_vsync;
  ...
  p_video->in_vsync = is_new_vsync;

  p_system_via = p_video->p_system_via;
  if (p_system_via) {
    via_set_CA1(p_system_via, is_new_vsync);
  }
  ...
  if (is_new_vsync) {
    ...
    p_video->last_vsync_raise_ticks = ticks;
  } else {
    teletext_VSYNC_changed(p_video->p_teletext, 0);

    p_video->last_vsync_lower_ticks = ticks;
  }
}
...
video_jump_to_vsync_start(struct video_struct* p_video) {
  ...
  p_video->is_odd_frame = (p_video->crtc_frames & 1);
  p_video->crtc_frames++;
  assert(!p_video->is_odd_vsync);
  assert(p_video->had_odd_vsync_this_row);
  assert(p_video->had_even_vsync_this_row);
  p_video->is_odd_vsync = 1;
  if (p_video->is_interlace) {
    if (p_video->crtc_frames & 1) {
      /* Jump to interlace even frame. */
      assert(p_video->horiz_counter == 0);
      assert(p_video->is_even_vsync);
      p_video->horiz_counter = p_video->half_r0;
    } else {
      /* Jump to interlace odd frame. */
  ...
  r0 = p_video->crtc_registers[k_crtc_reg_horiz_total];
  crtc_ticks = (p_video->vsync_pulse_width * (r0 + 1));
  return crtc_ticks;
}
...
video_timer_fired(void* p) {
  struct video_struct* p_video = (struct video_struct*) p;
  int timer_fire_mode = p_video->timer_fire_mode;
  ...
  if (timer_fire_mode >= k_video_timer_jump_to_vsync_raise) {
    uint32_t crtc_ticks;
    uint64_t ticks = timing_get_scaled_total_timer_ticks(p_video->p_timing);
    assert(!p_video->is_rendering_active);
    assert(p_video->has_sane_framing_parameters);
    ...
    (void) timing_set_timer_value(p_video->p_timing,
                                  p_video->timer_id,
                                  (crtc_ticks << p_video->clock_tick_shift));
  } else {
    video_advance_crtc_timing(p_video);
...
video_crtc_write_value(struct video_struct* p_video, uint8_t val) {
  ...
  case k_crtc_reg_interlace:
    ...
    ticks = timing_get_scaled_total_timer_ticks(p_video->p_timing);
    video_update_VSYNC(p_video, ticks);
    break;
  case k_crtc_reg_cursor_start:
